...
class Event (object):
  ...
  def _invoke (self, handler, *args, **kw):
    print ("DJ---/pox/lib/revent/revent.py Event._invoke", self, handler)
...
class EventMixin (object):
  ...
  def raiseEvent (self, event, *args, **kw):
    ...
    """
    print ("DJ---/pox/lib/revent/revent.py raiseEvent:%s!" %event)
    self._eventMixin_init()

    classCall = False
    if isinstance(event, Event):
      #print ("DJ---/pox/lib/revent/revent.py raiseEvent isinstance!")
      eventType = event.__class__
      classCall = True
      if event.source is None: event.source = self
    elif issubclass(event, Event):
      #print ("DJ---/pox/lib/revent/revent.py raiseEvent issubclass!",event)
      # Check for early-out
      if event not in self._eventMixin_handlers:
        print ("DJ---/pox/lib/revent/revent.py raiseEvent not in _eventMixin_handlers:",self,self._eventMixin_handlers)
        return None
      ...
      if event.source is None:
        event.source = self
    print("DJ---/pox/lib/revent/revent.py raiseEvent-2 raise",event ,eventType)
    ...
    print ("DJ---/pox/lib/revevt/revent.py raiseEvent-3 handlers",handlers)
    for (priority, handler, once, eid) in handlers:
      ...
      if classCall and event.halt:
        break
    #print ("DJ---/pox/lib/revevt/revent.py raiseEvent-4 event",handlers)
    return event
  ...
  def removeListener (self, handlerOrEID, eventType=None):
    ...
    print ("DJ---/pox/lib/revent/revent.py removeListener",handler)
  ...
  def addListener (self, eventType, handler, once=False, weak=False,
    ...
    if eventType not in self._eventMixin_handlers:
      ...
      handlers = self._eventMixin_handlers[eventType]
      #print ("DJ---/pox/lib/revent/revent.py already registered",handlers)
    ...
    #print ("DJ---/pox/lib/revent/revent.py addListener append",handlers)
    #print ("DJ---/pox/lib/revent/revent.py addListener ",self,self._eventMixin_handlers)
      ...
      if m.startswith("_handle" + prefix + "_"):
        ...
        if event in events:
          ...
          listeners.append(source.addListener(events[event], a, weak=weak,
                                              priority=priority))
          print("DJ---/pox/lib/revent/revent.py autoBind: ",source,m,"to",sink)
