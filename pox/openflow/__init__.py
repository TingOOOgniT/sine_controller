...
#DJ++ 20131219
from pox.lib.packet.packet_sine import sine
...
class FeaturesReceived (Event):
  ...
  """
  def __init__ (self, connection, ofp):
    print "DJ---/pox/openflow/__init__.py FeaturesReceived __init__!"
    Event.__init__(self)
    self.connection = connection
    self.dpid = connection.dpid
    self.ofp = ofp
...
class BarrierIn (Event):
  ...
  """
  def __init__ (self, connection, ofp):
    print "DJ---/pox/openflow/__init__.py event BarrierIn init!"
    Event.__init__(self)
    self.connection = connection
    self.ofp = ofp
    self.dpid = connection.dpid
...
class SINEIn (Event):
  def __init__ (self, connection, ofp):
    print "DJ---Event SINEIn __init__()!"
    ...
    # new variables for SINEIn Event

  def parse (self):
    if self._parsed is None:
      self._parsed = sine(self.data)
    return self._parsed
...
class OpenFlowNexus (EventMixin):
  ...
  def __init__ (self):
    print "DJ---/pox/openflow/__init__.py class OpenFlowNexus initiates!", self
