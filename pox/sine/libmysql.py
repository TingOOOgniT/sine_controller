...
tab_cap = namedtuple("table_capability", ("nid", "dpid", "cpu", "memory", "disk"))#need more infomation
tab_ser = namedtuple("table_service", ("sid", "nid"))
tab_pinfo = namedtuple("table_portinformation", ("nid", "port", "ip", "mac", "bandwidth" ))
...
class sinedb(object):
  ...
  def insert(self, table, *args):
    ...
    EXAMPLE:
    sinedb.insert("table_capability", (nid, dpid, '', mem, ''))
    Here the third column is for 'bandwidth' which allows "Null", but you still show give a "''".
    """

#    print "DJ---/pox/sine/libmysql.py insert table_service args:", args
...
#      print "DJ---/pox/sine/libmysql.py entering insert table_service!"
      for entry in args:
        if len(entry) != 3:
  ...
  def update(self, table, pk1, pk2 = None, **kw):
    ...
    Do like the example.
    EXAMPLE:
    sinedb.update("table_service", pk = xxx, sid=xxx, nid=sss)
  ...
  def delete(self, table, pk1, pk2 = None):
    ...
    EXAMPLE:
    sinedb.delete("table_capability", pk1=xxx)
    """
  ...
  def pk_select(self, table, sid = None, nid = None, port = None):
    ...
    print "DJ---/pox/sine/libmysql.py pk_select()", type(sid), sid, type(nid), nid
    if table == "table_capability":
      if not nid:
  ...
  #remember to close MySQL when POX goes down
  def closedb(self):
    try:
...
#daop:database operation-an instance of sindb for MySQL operation
dbop = sinedb()
#connect to database SINE
dbop.connect()
#dbop.cur.execute('show tables')
#en = dbop.cur.fetchall()
#sid = b'111111111111111111111111111111'
#nid = b'1111111111111111111111111'
#dbop.cur.execute("insert into table_service values(%s, %s, %s)", (sid,nid,''))
#ret = dbop.cur.execute("select * from table_service where sid=%s and nid=%s" % (sid, nid))
#print ret
